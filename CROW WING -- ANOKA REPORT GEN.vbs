	...
	Else 'Some staff may only have EK01 (MMIS MCRE). The script will allow workers to use that if applicable.
		...
		If row <> 0 then 
			If row <> 1 then
				...
				transmit
			End if
		Else 'Some OSAs have C410 (limited access). This will search for that.
			row = 1
			col = 1
			EMSearch "C418", row, col
			If row <> 0 then 
				If row <> 1 then
					EMWriteScreen "x", row, 4
					transmit
			...
			Else 'Some OSAs have EKIQ (limited MCRE access). This will search for that.
				...
				If row <> 0 then 
					...
					End if
				Else
					script_end_procedure("C418, C318, EKIQ, or EK01 not found. Your access to MMIS may be limited. Contact your script Alpha user if you have questions about using this script.")
				End if
			End if
		End if
	END IF
...
DO
    ...
    IF lep_tanf_check = 1 THEN MsgBox "The LEP TANF script exists only on the Anoka County server."
...
If all_workers_check = 1 then
	CALL create_array_of_all_active_x_numbers_in_county(x_array, "18")
Else
