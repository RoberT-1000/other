		...
		Else 'Some OSAs have C402 (limited access). This will search for that.
			...
			Else 'Some OSAs have EKIQ (limited MCRE access). This will search for that.
				...
				If row <> 0 then 
					...
					End if
				Else
					script_end_procedure("C419, C302, EKIQ, or EK01 not found. Your access to MMIS may be limited. Contact your script Alpha user if you have questions about using this script.")
				End if
			End if
		End if
	END IF
...
If all_workers_check = 1 then
	CALL create_array_of_all_active_x_numbers_in_county(x_array, "19")
Else
